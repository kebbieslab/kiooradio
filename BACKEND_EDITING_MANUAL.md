...
```bash
curl -X POST "https://radiokioo.preview.emergentagent.com/api/donations" \
  -H "Content-Type: application/json" \
  -d '{
...
curl "https://radiokioo.preview.emergentagent.com/api/your-new-endpoint"

# Test with data
curl -X POST "https://radiokioo.preview.emergentagent.com/api/your-endpoint" \
  -H "Content-Type: application/json" \
  -d '{"test": "data"}'
```
...
curl -w "@/dev/stdin" "https://radiokioo.preview.emergentagent.com/api/" <<< "
...
3. **Test New Feature**:
```bash
curl -X POST "https://radiokioo.preview.emergentagent.com/api/events" \
  -H "Content-Type: application/json" \
  -d '{
